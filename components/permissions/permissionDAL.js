...
exports.save = async (context) => {
  let item = new Permission(context);
...
exports.findById = async (id) => {
  item = await Permission.findOne({ _id: id });
  return item;
};

exports.update = async (id, item) => {
  // update req.body filter id
  const result = await Permission.updateOne({ _id: id }, { $set: item });
  return result;
};

exports.deleteById = async (id) => {
  item = await Permission.findOneAndDelete({ _id: id });
  return item;
};
