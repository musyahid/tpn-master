...
exports.getTotal = async () => {
  const result = await Role.countDocuments({});
  return result;
};
...
exports.findById = async (id) => {
  const pathMenu = {
    path: 'menus.menuId', select: ['name', 'level', 'icon', 'order'],
  };
  const pathMenus = {
    path: 'menus.childs', select: ['name', 'level', 'icon', 'order'],
  };
  const pathPermissions = {
    path: 'permissions', select: ['name', 'description', 'isMenu', 'url', 'editable'],
  };
  const item = await Role.findOne({ _id: id }).populate([pathMenu, pathMenus, pathPermissions]);
  return item;
};
exports.update = async (id, item) => {
  // update req.body filter id
  const result = await Role.updateOne({ _id: id }, { $set: item });
  return result;
};

exports.deleteById = async (id) => {
  item = await Role.findOneAndDelete({ _id: id });
  return item;
};
